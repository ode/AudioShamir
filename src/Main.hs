  ...
  case l of 
    "split":[filename, k', n'] -> do
      let 
        k = read k' :: Int
        n = read n' :: Int
      if k <= n && n > 0 && n < 256 then do
        wav <- readWaveFile filename
        h <- openFile filename ReadMode
        forM_  (map fromIntegral [1..n])  (\n -> writeWaveFile 
          (show n ++ filename)
        ...
        forM_ [1..(waveDataSize wav)] (\u -> do
          ...
          -- haha point-free form go brr
        putStrLn "done."
      else
        putStrLn "wrong k of n; please ensure k<=n, 0<n<256"
